...
$("[data-item=type-post]").on("change", function () {
    const type = $(this).val();
    $.getJSON("/Article/GetCategoryByTypePost", { type: type }, function (data) {
        var items = [];
        items.push("<option value>Chọn danh mục</option>");
        $.each(data, function (key, val) {
            items.push("<option value='" + val.Id + "'>" + val.Name + "</option>");
        });
        $("[data-item=category]").html(items.join(""));
    });
});
...
$("[data-item=root]").on("change", function (data) {
    ...
    if (id !== "") {
        $.getJSON("/ProductAdmin/GetProductCategory", { parentId: id }, function (data) {
            $.each(data,
                function (key, val) {
                    items.push("<option value='" + val.Id + "'>" + val.Name + "</option>");
                });
